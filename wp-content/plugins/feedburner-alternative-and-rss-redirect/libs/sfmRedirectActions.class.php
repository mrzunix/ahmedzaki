    ...
    public function sfmActivateRedirect()
    {
		...
		if(!empty($_POST['rtype']))
		{
			...
			switch($_POST['rtype'])
			{
				...
				break;    
				case "custom_rss"  :
					$web_url = (isset($_POST['curl']) && !empty($_POST['curl'])) ? trim($_POST['curl']) :'';
				...
				case "category_rss" :
					$web_url = html_entity_decode(get_category_feed_link(trim($_POST['record_id'])));
				...
				case "author_rss"   :
					$web_url = html_entity_decode(get_author_feed_link( trim($_POST['record_id'])));
    ...
    public function sfmReverseRedirect()
	{
		...
		if(isset($_POST['feed_id']) && !empty($_POST['feed_id']))
		{
			switch($_POST['feed_type'])
			{
				...
				break;
				case "category_rss" :
					$fdata= $this->sfmGetRssDetailByFeed(array($_POST['feed_id']));
