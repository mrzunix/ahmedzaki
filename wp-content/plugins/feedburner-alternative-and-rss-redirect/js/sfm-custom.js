...
SFM( document ).ready(function( $ )
{
   	...
   	SFM('body').on('click', '.activate_redirect', function()
	{
        ...
        switch(redirection_type)
        {
            case "main_rss" :
				...
				var data={action:'ActRedirect',rtype:redirection_type}
            break;
           	case "comment_rss" :
            ...
            break;
          	case "category_rss" :
				...
				var data={action:'ActRedirect',rtype:redirection_type,record_id :cat_id }
            break;
         	case "author_rss" :
				SFM(element).addClass('sfrd_process_small');
				SFM(element).text('Applying redirect..');
				...
				var data={action:'ActRedirect',rtype:redirection_type,record_id :author_id }
            break;
       	 	case "custom_rss" :
    ...
    SFM('body').on('click', '.reverse_redirect,.reverse_redirect1', function()
	{
      ...
      var data={action: "sfmReverseRedirect",feed_id:feed_id,feed_type:redirection_type}
      SFM.ajax({
                url: ajax_object.ajax_url,
                type:'post',
                data: data,
...
function ProcessAfterCustomActive(res)
{
    ...
    content+='<span> <a id="custom_'+res.rid+'" target="_new" title="'+res.feed_url+'" href="'+res.feed_url+'">'+res.feed_url+'</a></span>'
    content+='</h3>';
    content+="<small>";
    content+='<a class="open_new_feed1" title="Open the new feed " target="_new" href="'+res.feed_url+'">Open the new feed</a>';
...
{
	var data={feed_id: feed_id,'action': 'sfmProcessFeeds'}
  	SFM.ajax({
		url: ajax_object.ajax_url,
		type:'post',
		data: data,
