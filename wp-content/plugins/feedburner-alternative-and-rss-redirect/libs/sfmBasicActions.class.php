...
class sfmBasicActions
{
    ...
    public function sfmListActiveRss()
    {
        ...
        /* get the authors */
        global $wpdb;
        $wp_authors=$wpdb->get_results('select p.post_author,u.user_login from '.$wpdb->prefix.'posts p LEFT JOIN '.$wpdb->prefix.'users u on p.post_author=u.ID where p.post_status="publish" and p.post_type="post"',ARRAY_A);       
